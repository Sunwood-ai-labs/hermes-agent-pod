...
- 既定の inference provider は `zai`、既定 model は `glm-5.1`、base URL は Z.AI GLM Coding Plan endpoint です。
...
最近の Hermes Agent image には SQLite-backed の Kanban board が入っています。この repo は dashboard surface を公開するため、Codex が host 側の変更管理を握ったまま、browser から board を確認できます。

![Hermes Kanban dashboard](docs/public/hermes-kanban-dashboard.png)

別 PC から LAN 内だけで見る場合は、local `.env` に次を追加します。Dashboard だけを信頼できる local interface に bind し、gateway API は localhost のままにします。
...
別 PC から `http://192.168.11.200:19119` を開きます。IP address はこの machine の LAN address に置き換えてください。
...
Z.AI GLM Coding Plan key を Compose と、kind が使える場合は kind Secret に設定:
